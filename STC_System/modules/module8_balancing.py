...
class PortfolioBalancingModule:
    ...
    def run(
        ...
        if is_chunk_mode:
            # 1. حساب عدد العملاء لكل محصل في المحفظة المصدر حالياً
            source_counts = {c: collector_lookup[c]["cnt"] for c in human_collectors_normalized}
            source_balances = {c: collector_lookup[c]["bal"] for c in human_collectors_normalized}
            total_source_clients = sum(source_counts.values())
            
            # 2. تحديد عدد المحصلين النشطين المسموح به في هذه المحفظة
            ...
            # 3. اختيار المحصلين النشطين (الأعلى كفاءة وعدد عملاء ورصيد حالي)
            ...
            # 4. توزيع الهدف بالتساوي على المحصلين النشطين فقط
            ...
            # 5. تحديد المانحين والمستقبلين بناءً على مستهدفات المحفظة المصدر
            givers = []
            receivers = []
            for c in human_collectors_normalized:
                diff = target_cnt_source[c] - source_counts[c]
                if diff < 0:
                    givers.append((c, -diff))   # فائض يجب سحبه
                elif diff > 0:
                    receivers.append((c, diff)) # نقص يجب إكماله
                    
            # للاستخدام في التقارير
            target_cnt = {c: (collector_lookup_before[c]["cnt"] - source_counts[c] + target_cnt_source[c]) for c in human_collectors_normalized}
            ideal_cnt = total_source_clients / max_active
            ideal_bal = total_bal / max_active
        else:
