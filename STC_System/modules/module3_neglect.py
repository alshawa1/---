...
class NeglectModule:
    ...
    def run(self, portfolio: pl.DataFrame) -> Dict:
        ...
        # ─── استبعاد الحالات الرئيسية المحددة قبل البدء ────────────────────
        # (عدم توصل - مسجون - اعتراض - متوفي - خروج نهائي - تسوية)
        if status_main_col in df.columns:
            exclude_list = ["عدم توصل", "مسجون", "متوفي", "خروج نهائي", "تسوية", "تسويه"]
            df = df.filter(
            ...
            _log.info("🚫 تم استبعاد حالات (عدم توصل - مسجون - اعتراض - متوفي - خروج نهائي - تسوية) - المتبقي: %d صف", len(df))
